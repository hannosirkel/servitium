...
const ludusRoot = path.join(__dirname, '..', 'dist', 'ludus');
const mimeTypes = {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    nav { display: grid; grid-template-columns: repeat(4, minmax(190px, 260px)); gap: 12px; }
  ...
  <main>
    ...
    <section class="content">
      ...
      <nav aria-label="Applications">
        ...
        <a href="/ludus/" aria-label="Ludus — Games"><span class="icon" aria-hidden="true">♜</span><span><b>Ludus</b><small>PLAY A QUIET GAME</small></span></a>
      </nav>
    </section>
  </main>
</body>
...
function createServer() {
  return http.createServer((request, response) => {
    ...
    if (request.method === 'GET' && request.url === '/ludus') {
      response.writeHead(308, { location: '/ludus/' });
      response.end();
      return;
    }
    ...
    if (request.method === 'GET' && request.url.startsWith('/ludus/')) {
      ...
      if (filePath.startsWith(`${ludusRoot}${path.sep}`) && fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
        const body = fs.readFileSync(filePath);
        const immutable = relativePath !== 'ludus.html';
        send(response, 200, mimeTypes[path.extname(filePath)] || 'application/octet-stream', body,
          immutable ? 'public, max-age=31536000, immutable' : 'no-cache');
        return;
      }
